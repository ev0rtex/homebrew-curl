class CurlAres < Formula
  ...
  keg_only "this is a custom build of curl with c-ares and preferrably we don't conflict with the system version of curl"
  ...
  def install
    ...
    inreplace "scripts/wcurl", 'CMD="curl "', "CMD=\"#{opt_bin}/curl-ares \""
  ...
  test do
    ...
    system bin/"curl-ares", "-L", stable.url, "-o", filename
    filename.verify_checksum stable.checksum

    # Verify QUIC and HTTP3 support
    system bin/"curl-ares", "--verbose", "--http3-only", "--head", "https://cloudflare-quic.com"

    # Check dependencies linked correctly
    curl_features = shell_output("#{bin}/curl-config-ares --features").split("\n")
    %w[AsynchDNS brotli GSS-API HTTP2 HTTP3 IDN libz SSL zstd].each do |feature|
      assert_includes curl_features, feature
    end
    curl_protocols = shell_output("#{bin}/curl-config-ares --protocols").split("\n")
